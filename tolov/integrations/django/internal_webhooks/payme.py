            ...
            return JsonResponse(
                {
                    ...
                    "error": {"code": PaymeErrors.AUTH_ERROR, "message": str(e)},
                },
                status=200,
            )
            ...
            return JsonResponse(
                {
                    "jsonrpc": "2.0",
                    "id": request_id if "request_id" in locals() else 0,
                    "error": {"code": PaymeErrors.METHOD_NOT_FOUND, "message": str(e)},
                },
                status=200,
            )
            ...
            return JsonResponse(
                {
                    "jsonrpc": "2.0",
                    "id": request_id if "request_id" in locals() else 0,
                    "error": {"code": PaymeErrors.INVALID_ACCOUNT, "message": str(e)},
                },
                status=200,
            )
            ...
            return JsonResponse(
                {
                    "jsonrpc": "2.0",
                    "id": request_id if "request_id" in locals() else 0,
                    "error": {"code": PaymeErrors.INVALID_AMOUNT, "message": str(e)},
                },
                status=200,
            )
    ...
    def _create_transaction(self, params):
        ...
        if self.one_time_payment:
            existing_transactions = PaymentTransaction._default_manager.filter(
                gateway=PaymentTransaction.PAYME, account_id=account.id
