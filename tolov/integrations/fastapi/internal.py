"""
Internal FastAPI webhook handlers for Tolov.
...
class PaymeWebhookHandlerInternal(BasePaymentProcessor):
                ...
                content=json.dumps(
                    {
                        ...
                        "error": {"code": -32504, "message": "permission denied"},
                    }
                ),
                media_type="application/json",
                status_code=200,
                ...
                content=json.dumps(
                    {
                        "jsonrpc": "2.0",
                        "id": request_id if "request_id" in locals() else 0,
                        "error": {"code": -31050, "message": str(e)},
                    }
                ),
                media_type="application/json",
                status_code=200,
    ...
    def _find_account(self, params: Dict[str, Any]) -> Any:
        ...
        account = (
            ...
            .filter_by(**{lookup_field: account_value})
            .first()
        )
        if not account:
...
class ClickWebhookHandlerInternal:
    ...
    def _find_account(self, merchant_trans_id: str) -> Any:
        ...
        account = self.db.query(self.account_model).filter_by(**lookup_kwargs).first()
        if not account:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
